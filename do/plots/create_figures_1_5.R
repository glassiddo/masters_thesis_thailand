...
g1 <- grid |> 
  ...
  filter(region_type == "Northern Thailand") |> 
  summarise(
    n_fires = mean(total_fires_modis),
    .by = c(year, month, region_type)
  ) 
...
ggplot(
  ...
  theme(
    ...
    legend.key.width = unit(2.5, "cm"),  # Wider legend from second version
...
f_with_shares <- modis_fires |> 
  ...
  summarise(
    n = n(), 
    .by = c(time_midpoint, SATELLITE, period)
    ) |> 
